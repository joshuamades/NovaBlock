...
**Source cross-check:** Phaser source commit `8a01674ddaf78f222758d17cfd2dfa290240f808`, especially `src/animations/AnimationManager.js`, `src/animations/AnimationState.js`, `src/animations/Animation.js`, and `src/animations/events/index.js`.
...
### Source-Backed Runtime Model

From Phaser's animation source:

- `AnimationManager` is created by the Game and is accessed from a Scene as `this.anims`. It owns global animation definitions, the global `globalTimeScale`, the global paused flag, and the animation mix map.
- `AnimationState` is created per animated Game Object, such as a Sprite. It owns playback state: `isPlaying`, `hasStarted`, `currentAnim`, `currentFrame`, `nextAnim`, `nextAnimsQueue`, `timeScale`, direction flags, delay/repeat counters, and pending-stop state.
- `Animation` owns the immutable-ish frame sequence and default timing config. During playback it advances an `AnimationState` by calling `nextFrame`, `previousFrame`, `repeatAnimation`, and `complete` paths.
- `AnimationFrame` entries are linked in a loop: each frame has `nextFrame`, `prevFrame`, `isFirst`, `isLast`, and `progress`. When frames are added or removed, `updateFrameSequence()` rebuilds those links.

Practical implication: create reusable definitions once on `this.anims`, but control playback through `sprite.anims` or shorthand methods like `sprite.play()`.
...
- Runtime playback uses `msPerFrame = 1000 / frameRate`.
- A frame-specific `duration` overrides the next tick only when the active playback frame rate matches the animation's default frame rate. If playback overrides `frameRate`, Phaser uses `msPerFrame`.
- The animation update loop accumulates `delta * sprite.anims.timeScale * this.anims.globalTimeScale`, so both per-sprite and global time scale affect frame advancement.

### Source-Backed Playback Details

These details matter when debugging animation timing and events:

- `skipMissedFrames: true` lets Phaser advance through multiple frames in one update if accumulated time is ahead. The internal loop has a safety cap, so it will not spin forever.
- `delay` uses `delayCounter`. If `showBeforeDelay` is `true`, the first frame can be shown before playback officially starts.
- `repeat` is stored as `repeatCounter`; `repeat: -1` is treated as infinite and will not naturally complete.
- `yoyo` changes playback direction before repeat/complete handling. In forward playback, reaching the last frame can yoyo backward before repeating or completing.
- `repeatDelay` delays the repeat transition by extending the next tick before decrementing the repeat counter.
- `playAfterDelay` sets a delayed next animation. `playAfterRepeat` queues an animation after a repeat count. These paths bypass mix-delay behavior.
- `stopAfterDelay`, `stopAfterRepeat`, and `stopOnFrame` are tracked by a pending-stop mode. They fire stop behavior, not natural complete behavior.
- `setCurrentFrame()` applies the texture/frame to the Game Object, updates crop UVs when cropped, calls `setSizeToFrame()`, updates origin if needed, emits animation update events while playing, and checks `stopOnFrame`.
- If a global animation is removed while a Sprite is using it, the Sprite's `AnimationState` receives the remove event and must stop or clear references.
...
Event constants in Phaser's source are exported from `src/animations/events/index.js`. The central event set is: add animation, remove animation, start, update, repeat, restart, stop, complete, keyed complete, pause all, and resume all.
